...
public class ItemServiceImpl implements ItemService {
    ...
    public ItemDto create(Long userId, ItemDto dto) {
        ...
        if (dto.getRequestId() != null) {
            request = itemRequestRepository.findById(dto.getRequestId()).orElseThrow(() -> new NotFoundException("request not found"));
        }
        Item item = ItemMapper.toItem(dto, user, request);
    ...
    public ItemDto getById(Long userId, Long itemId) {
        ...
        dto.setComments(
                commentRepository.findByItemIdOrderByCreatedDesc(itemId).stream()
    ...
    private void setBookingsForItem(ItemDto itemDto, Long requesterId) {
        ...
        bookingRepository.findFirstByItemIdAndStartBeforeAndStatusOrderByStartDesc(item.getId(), now, BookingStatus.APPROVED).ifPresent(b -> {
        ...
        bookingRepository.findFirstByItemIdAndStartAfterAndStatusOrderByStartAsc(item.getId(), now, BookingStatus.APPROVED).ifPresent(b -> {
    ...
    public CommentDto addComment(Long userId, Long itemId, CommentDto dto) {
        ...
        LocalDateTime now = LocalDateTime.now();
        List<Booking> bookings = bookingRepository.findPastApprovedBookingForComment(userId, itemId, now);
        if (bookings.isEmpty()) {
